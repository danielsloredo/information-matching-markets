...
def generate_candidates(student_preferences, n_candidates): 
...
def gale_shapley_modified(n_students, n_schools, student_preferences, school_preferences):
    ...
    while unmatched_students: 
        ...
        size_preferences = student_preferences[student].shape[0]
...
def simulation_matching_increase_preferences(Delta, k, n_students, n_schools, additions, n_candidates):
    '''
    ...
    for j in range(1,additions+1):
        ...
        student_pre[k], school_pre[k] = counselor_increase_preference_sublist(Delta, student_pre[k_prev], school_pre[k_prev], student_f_pref, school_f_pref, candidates)
        student_M[k], school_M[k] = gale_shapley_modified(n_students, n_schools, student_pre[k], school_pre[k])
...
def mc_simulations_improvement(Delta, sublist, additions, n_students, n_schools, n_candidates, iterations):
    ...
    return (average_num_students_change, average_num_schools_change, 
    average_num_students_unm_mat, average_num_schools_unm_mat, 
